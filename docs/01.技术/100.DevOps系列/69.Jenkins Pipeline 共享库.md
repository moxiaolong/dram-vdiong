---
title: Jenkins Pipeline 共享库 categories:
...
- 运维
- jenkins
- pipeline date: 2022-04-11 22:03:00 permalink: /pages/devops/jenkins/pipeline/shared-library
...
def call() {
    ...
    try {
        ...
        podTemplate(label: env.label, cloud: 'kubernetes', containers: [containerTemplate(name: 'jnlp',
                ...
                volumes: [hostPathVolume(mountPath: '/root/workspace/', hostPath: '/jenkin-agent/workspace'),
                          hostPathVolume(mountPath: '/var/lib/docker/image/', hostPath: '/jenkin-agent/dockerimage')
                ]) {
